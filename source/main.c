...
int main()
{
    ...
    qSensorData = xQueueCreate( 100, sizeof(sensor_data_t) );
    if (qSensorData == NULL)
    {
    	NVIC_SystemReset();
    }

// Set this flag to collect data (Set in Makefile)
#if (DATA_COLLECTION)
    /* Create the tasks. */
    xTaskCreate(tcp_client_task, "Network Data Collection task", TCP_CLIENT_TASK_STACK_SIZE, NULL,
                TCP_CLIENT_TASK_PRIORITY, NULL);

#else
    /* Create one task, processes all sensor data and feeds it to the inference engine */
    xTaskCreate(sensor_task, "Sensor Processing", TASK_SENSOR_STACK_SIZE, NULL, TASK_SENSOR_PRIORITY, NULL);
#endif
