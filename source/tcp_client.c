...
#define MAKE_IP_PARAMETERS(a, b, c, d)           ((((uint32_t) d) << 24) | \
	...
	#define FIXED_IP_ADDR						  "192.168.8.100"
    #define TCP_SERVER_PORT                           (8085)
...
#define THROUGPUT_BUFFER						  (1460u * 1u)
...
static SemaphoreHandle_t connect_to_server;
...
//        cyhal_syspm_unlock_deepsleep();
        ...
        ip4addr_aton((char *)FIXED_IP_ADDR,
...
void tcp_transmit_recieve(void)
{
	...
	uint32_t bytes_sent = 0, bytes_received = 0;
	sensor_data_t sensorData;

	while(1)
	{
		xQueueReceive(qSensorData, &sensorData, portMAX_DELAY);
		result = cy_socket_send(client_handle, sensorData.pData, sensorData.dataSize,
...
//			printf("Send Success\r\n");
	    ...
	    result = cy_socket_recv(client_handle, message_buffer, 1,
...
//			printf("Received: %s\r\n", message_buffer);
    ...
    printf("Disconnecting from the TCP server! \n");
